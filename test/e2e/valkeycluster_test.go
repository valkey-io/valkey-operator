...
`, singleNodeClusterName)
		...
		It("creates a cluster with custom users", func() {
			...
			verifyAllowedPermissionsOfOperatorUser := func(g Gomega) {
				...
				cmd := exec.Command(
					"sh", "-c",
					fmt.Sprintf(`kubectl get secret internal-%s-system-passwords \
    -n default \
    -o jsonpath='{.data._operator}' | base64 -d`, withUserClusterName),
				)
				operatorPassword, err := utils.Run(cmd)
				g.Expect(err).NotTo(HaveOccurred())
...
EOF`,
    ...
    -o jsonpath='{.data._operator}' | base64 -d`, withUserClusterName),
				...
				for _, command := range disallowedCommands {
					...
					g.Expect(output).To(ContainSubstring("NOPERM"),
