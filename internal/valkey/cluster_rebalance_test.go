...
func TestPlanRebalanceMove_ShardCountMismatch(t *testing.T) {
	...
	if move != nil {
		t.Fatalf("expected nil when shard count != expectedShards, got %+v", move)
	}
}
...
func TestPlanDrainMove_Basic(t *testing.T) {
	...
	move, err := PlanDrainMove(src, dsts, 100)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if move == nil {
...
func TestPlanDrainMove_PicksFirstDst(t *testing.T) {
	...
	move, err := PlanDrainMove(src, dsts, 50)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
