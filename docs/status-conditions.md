...
#### `WorkloadDrift`
Indicates that the ValkeyNode controller has built a pod template that differs from the live StatefulSet or Deployment, but `spec.workloadRevision` does not yet authorize that template.
...
| Status | Meaning |
|---|---|
| `True` | Desired template hash does not match `spec.workloadRevision`; rolling update is deferred. |
| `False` or absent | No deferred workload template update. |

Common reasons when `WorkloadDrift=True`:
