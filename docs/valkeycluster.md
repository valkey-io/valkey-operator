...
`serverName` is the hostname the operator verifies when it dials a node by pod IP (TLS SNI / cert SAN). When unset, it uses `valkey-<name>.<namespace>.svc.<clusterDomain>` (default `cluster.local`) **without** a trailing dot ([RFC 6066 §3](https://www.rfc-editor.org/rfc/rfc6066#section-3)). Hostname announce may still use the absolute FQDN (`….svc.<clusterDomain>.`). The cluster writes that resolved SNI name onto each `ValkeyNode`; the node client and the metrics exporter (`REDIS_EXPORTER_TLS_SERVER_NAME`) use it as-is. The exporter still dials `localhost`. This does not change what nodes announce in `CLUSTER SLOTS`.
