...
- `MultipleConfigurationWarnings` – more than one configuration warning is active at the same time. The condition message carries a single reason, so the message lists all of them, and an event is emitted per warning.
